...
setting_event_prune_backend = setting_namespace.do_setting_add(
    ...
    help_text=_(
        message='Path to the event pruning subclass that will be called '
...
setting_event_prune_backend_arguments = setting_namespace.do_setting_add(
    ...
    help_text=_(message='Arguments to pass to `EVENTS_PRUNE_BACKEND`.')
)
setting_event_prune_task_interval = setting_namespace.do_setting_add(
    ...
    help_text=_(
        message='Time interval in seconds, at which the event prune task '
        'will execute.'
    )
)
