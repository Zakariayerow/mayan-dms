...
def code_document_file_document_index_create(apps, schema_editor):
    DocumentFile = apps.get_model(
        app_label='documents', model_name='DocumentFile'
    )

    field = DocumentFile._meta.get_field(field_name='document')
...
def code_document_file_document_index_delete(apps, schema_editor):
    DocumentFile = apps.get_model(
        app_label='documents', model_name='DocumentFile'
    )
...
class Migration(migrations.Migration):
    dependencies = [
        ('documents', '0094_clear_restored_document_trashed_date_time')
    ]

    operations = [
        migrations.RunPython(
            code=code_document_file_document_index_create,
            reverse_code=code_document_file_document_index_delete
        )
    ]
